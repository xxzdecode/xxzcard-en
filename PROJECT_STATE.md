...
- Service Worker 当前为 `xxzcard-app-shell-v66` / `xxzcard-runtime-v66`，老师首页摘要、金币调整和错题整理资源均已进入应用壳缓存；Supabase 请求不再被 Service Worker 额外缓存或以 5 秒超时中断，后台全量镜像刷新失败也不会禁用小键读写与错题保存；
