...
- Service Worker 当前为 `xxzcard-app-shell-v99` / `xxzcard-runtime-v99`；Supabase 请求不由 Service Worker 额外缓存，苹果端读取超时为 12 秒并保留自动重试；浏览器已停止整表下载 `kv_store`、首页完整 `main` 和 `pre_*` 历史快照；`device-diagnostics.html` 可分别测量首页摘要与用户主动触发的完整词库读取，不输出账号、词库或答题内容；错题批改记录可保存一段可选的老师补充说明并支持桌面拖入图片上传；
