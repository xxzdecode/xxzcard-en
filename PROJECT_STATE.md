...
- 第二层根因确认：Supabase `main` 响应约 593 KB，其中 `masterCards` 约 500 KB；此前即使首页已有本地内容，后台仍会读取完整 `main`，苹果 WebKit 链路偶发超过超时阈值并显示“云端连接暂时较慢”；
- 首页启动现在只读取约 2.4 KB 的 JSONB 摘要字段，不读取或解析完整词库；完整 `main` 只在首次进入词汇探险、单词挑战、单词卡、新词导览或摸底功能时加载，并在本次会话内复用；
- 后台 GET 变慢不再把整个应用误判成离线；浏览器明确断网和写入失败仍保留只读保护与提示；
- 老师端和学生端单词本列表在入口加载完整主数据后立即显示，并复用本地镜像作为断网兜底；
...
- Service Worker 当前为 `xxzcard-app-shell-v100` / `xxzcard-runtime-v100`；Supabase 请求不由 Service Worker 额外缓存，苹果端读取超时为 12 秒并保留自动重试；浏览器已停止整表下载 `kv_store`、首页完整 `main` 和 `pre_*` 历史快照；`device-diagnostics.html` 可分别测量首页摘要与用户主动触发的完整词库读取，不输出账号、词库或答题内容；错题批改记录可保存一段可选的老师补充说明并支持桌面拖入图片上传；
- 新词导览已改为分类优先：首页直接显示主题词汇、功能词汇、按四/七年级和日期组织的校内词汇，以及总库未分类；分组直接显示在类别下，完成项以浅绿色和小勾标记并后置。旧普通单词本只从导览隐藏，老师端“＋ 新建单词本”和手动粘贴导入仍保留。
- 已增加静态、数据层和浏览器回归断言，覆盖首页零完整词库请求、首次词汇入口单次完整请求、缓存兜底、学习写入安全边界及 WebKit 冷启动；CI 状态必须按当前提交或 PR 实时核对，不沿用旧快照结论；
