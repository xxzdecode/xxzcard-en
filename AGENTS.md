...
- 收到“上传练习”时，不在本仓库自行猜测待上传文件；唯一详细流程位于 `xxzdecode/xxz-tools` 的 `english-project/README.md`。
- 未找到有效的 `english-project/inbox/待上传练习.md` 时立即停止，不上传旧练习或测试副本。若交接来自迁移前流程，可兼容读取旧文件名 `待上传课件.md`，但不得继续把它作为主名称。
