...
- 涉及页面设计、响应式布局、图片素材、视觉截图或视口测试时，必须先读取 `docs/device-viewport-contract.md`。唯一正式验收设备固定为：iPad Air 11 英寸横屏 `1180 × 820` CSS 视口，以及 iPhone 16 竖屏 `393 × 852` CSS 视口；除非用户当次明确要求，不再把其他设备、尺寸或方向列为必须适配与验收范围。
...
- 默认使用 `scripts/publish-vocabulary-review-images.py prepare` 将当前来源的全部 `pending` 批次作为一次发布统一接入和检查，只做一次精确提交与推送；推送成功后再用 `finalize` 将参与批次统一改为 `completed`。不得默认拆成逐批提交、逐批推送。
- 处理前核对图片、清单和文件映射；发现缺图、重复、串图、文字或明显无效图片时停止对应批次，不做猜测式接入。
- 完成回报必须包含实际来源、处理批次与数量、检查结果、提交 SHA、推送结果、部署或线上核验状态，以及各批次状态更新。不得把“本地改完”“已提交”或“已推送”直接说成“已上线”。
