...
- 收到“上传生词巩固图片”时，必须读取 `xxzdecode/xxz-tools/xxzcard-en-hub/rules/current/11_生词巩固图片生成与上传流程.md`，并默认只处理 `/mnt/data/vocabulary-review-images/` 中最新且状态为 `pending` 的有效本地批次；本地目录不可见或没有有效批次时立即停止，不使用 GitHub 历史批次、旧图或自行猜测。只有用户明确要求云端中转时，才读取 `xxzcard-en-hub/inbox/vocabulary-review-images/`。
