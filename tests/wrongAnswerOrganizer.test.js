...
assert.match(html, /id="wrongAnswerMediaDropZone"/);
...
assert.match(styles, /\.wrong-answer-media__upload\.is-dragover/);
...
assert.match(organizerSource, /mediaDropZone\.addEventListener\('dragover'/);
assert.match(organizerSource, /mediaDropZone\.addEventListener\('drop'/);
assert.match(organizerSource, /uploadMediaFiles\(event\.dataTransfer\?\.files \|\| \[\]\)/);
