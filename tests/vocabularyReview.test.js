...
assert.match(task016Script, /if \(isTransientBatch\(batch\)\) return true;/);
...
assert.match(task016Script, /loadVocabularyLessonTaughtState/);
assert.match(task016Script, /saveVocabularyLessonTaughtGroup/);
assert.match(task016Script, /isVocabularyLessonGroupTaught/);
assert.match(task016Script, /已授课/);
assert.doesNotMatch(task016Script, /wc_vocabulary_lesson_position_v1:/);
assert.doesNotMatch(task016Script, /vocab_lesson_progress_v1_/);
assert.doesNotMatch(task016Script, /localStorage\.setItem/);
...
assert.match(task016Script, /const result = baseOpenList\(\);\s*loadTaughtState\(\{ fresh: true, silent: true \}\)/);
assert.doesNotMatch(task016Script, /loadRelevantCloudProgress|saveCurrentCloudPosition/);
