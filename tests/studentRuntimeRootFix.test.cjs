...
assert.match(queueSource, /getVocabularyLessonAvailableCategoryGroups/);
assert.match(queueSource, /collectVocabularyGuideCandidates/);
assert.doesNotMatch(queueSource, /mergeFormalChallengeCandidates/);
...
assert.match(serviceWorker, /xxzcard-app-shell-v104/);
assert.match(serviceWorker, /xxzcard-runtime-v104/);
...
assert.doesNotMatch(serviceWorker, /courseware\/26\.08\.04｜时间介词 in-on-at 随堂练习\.html/);
assert.doesNotMatch(serviceWorker, /courseware\/26\.08\.06｜地点介词“上与下”随堂练习\.html/);
assert.doesNotMatch(serviceWorker, /'\.\/grammar-challenge\/data\/2026-07-16\.js'/);
