...
const oldWorker = `
const CACHE = 'xxzcard-app-shell-v92';
    ...
    latest = await page.evaluate(async expected => {
      ...
      return {
        ...
        ready: registration?.active?.state === 'activated'
          ...
          && (!/v93$/.test(expected) || !keys.some(key => /v92$/.test(key)))
      };
    }, cacheName);
...
try {
  ...
  await waitForActiveVersion(page, 'xxzcard-app-shell-v92');
  ...
  const cacheState = await page.evaluate(async () => {
    const keys = await caches.keys();
    const shell = await caches.open('xxzcard-app-shell-v93');
    const urls = (await shell.keys()).map(request => decodeURIComponent(new URL(request.url).pathname));
    return { keys, urls };
  });
  assert.ok(
    !cacheState.keys.some(key => /v92$/.test(key)),
    `v92 caches removed after activation: ${JSON.stringify(cacheState.keys)}`
  );
  assert.equal(cacheState.urls.length, 24, 'v93 Apple-safe app shell must contain exactly 24 resources');
  ...
  await page.goto(`${baseUrl}/?cold=v93`, { waitUntil: 'domcontentloaded' });
  ...
  assert.deepEqual(finalCaches.sort(), ['xxzcard-app-shell-v93', 'xxzcard-runtime-v93']);
  console.log(`student runtime v92-to-v93 cold-start viewport tests passed (${process.env.PW_BROWSER === 'webkit' ? 'WebKit' : 'Chromium'})`);
} finally {
