...
const oldWorker = `
const CACHE = 'xxzcard-app-shell-v89';
    ...
    latest = await page.evaluate(async expected => {
      ...
      return {
        ...
        ready: registration?.active?.state === 'activated'
          ...
          && (!/v90$/.test(expected) || !keys.some(key => /v89$/.test(key)))
      };
    }, cacheName);
...
try {
  ...
  await waitForActiveVersion(page, 'xxzcard-app-shell-v89');
  ...
  await waitForActiveVersion(page, 'xxzcard-app-shell-v90');

  const cacheState = await page.evaluate(async () => {
    const keys = await caches.keys();
    const shell = await caches.open('xxzcard-app-shell-v90');
    const urls = (await shell.keys()).map(request => decodeURIComponent(new URL(request.url).pathname));
    return { keys, urls };
  });
  assert.ok(
    !cacheState.keys.some(key => /v89$/.test(key)),
    `v89 caches removed after activation: ${JSON.stringify(cacheState.keys)}`
  );
  assert.equal(cacheState.urls.length, 24, 'v90 Apple-safe app shell must contain exactly 24 resources');
  ...
  await page.goto(`${baseUrl}/?cold=v90`, { waitUntil: 'domcontentloaded' });
  ...
  assert.deepEqual(finalCaches.sort(), ['xxzcard-app-shell-v90', 'xxzcard-runtime-v90']);
  console.log(`student runtime v89-to-v90 cold-start viewport tests passed (${process.env.PW_BROWSER === 'webkit' ? 'WebKit' : 'Chromium'})`);
} finally {
